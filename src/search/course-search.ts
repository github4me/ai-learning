...
function tokens(value: string, useSegmenter: boolean): string[] {
  ...
  const base = normalized.split(/[^\p{L}\p{N}\u3400-\u9fff\uf900-\ufaff-]+/u).filter(Boolean);
  const output = new Set(base.flatMap((term) => [term, term.replaceAll('-', '')]));
  const cjkRuns = normalized.match(/[\u3400-\u9fff\uf900-\ufaff]+/gu) ?? [];
  if (useSegmenter && typeof Intl.Segmenter !== 'undefined') {
    const segmenter = new Intl.Segmenter('zh', { granularity: 'word' });
    for (const run of cjkRuns) for (const part of segmenter.segment(run)) if (part.isWordLike) output.add(part.segment);
  }
  for (const run of cjkRuns) {
    const points = Array.from(run);
    output.add(run);
    if (!useSegmenter) for (let index = 0; index < points.length - 1; index += 1) output.add(points.slice(index, index + 2).join(''));
  }
  return [...output];
}
function inlineText(nodes: InlineNode[]): string {
  return nodes.map((node) => 'children' in node ? inlineText(node.children) : node.value).join('');
}
function blockText(block: ContentBlock): string {
  switch (block.type) {
    case 'paragraph': return inlineText(block.children);
    case 'list': return block.items.map(inlineText).join('\n');
    case 'formula': return `${block.accessibleText}\n${block.latex}`;
    case 'code': return `${block.filename ?? ''}\n${block.language}\n${block.code}`;
    case 'table': return [block.caption ?? '', ...block.headers, ...block.rows.flat()].map((row) => Array.isArray(row) ? inlineText(row) : row).join('\n');
    case 'callout': return `${block.title ?? ''}\n${block.blocks.map(blockText).join('\n')}`;
    case 'conceptChain': return block.steps.join(' → ');
    case 'knowledgeCheck': return `${inlineText(block.prompt)}\n${(block.answer ?? []).map(blockText).join('\n')}`;
  }
}
function escapedExcerpt(text: string, query: string): string {
  const clean = text.replace(/\s+/g, ' ').trim();
  const normalized = normalize(clean);
  const needle = tokens(query, false).find((token) => token.length > 1) ?? normalize(query);
  const at = Math.max(0, normalized.indexOf(needle));
  const start = Math.max(0, at - 40);
  const points = Array.from(clean).slice(start, start + 140).join('');
  return points.replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;').slice(0, 140);
}
...
export function createCourseSearch(course: Course, options: { segmenter?: boolean } = {}): CourseSearchIndex {
  ...
  for (const root of roots) {
    const unit = root as typeof root & { kind?: 'week' | 'appendix'; weekNumber?: number };
    for (const section of flattenSections(root).filter((item) => item.showInToc)) {
      const glossary = (course.glossary ?? []).filter((entry) => entry.sectionId === section.id).flatMap((entry) => [entry.term, entry.definition, ...(entry.aliases ?? [])]).join('\n');
      const rawText = [section.title, root.title, ...section.blocks.map(blockText), glossary].filter(Boolean).join('\n');
