        ...
        table(
          ...
          [
            [
              'B：3 → 1',
              '每一步只装入一条样本',
              '模型结构不变；并行量、内存和一次 Gradient 使用的样本数改变',
            ],
            [
              'T：2 → 4',
              '每条样本保留四个有顺序的位置',
              '能处理更长的当前窗口；Attention 的位置间计算也会增加',
            ],
            [
              'C：4 → 8',
              '每个位置从四个内部数扩为八个',
              '表示宽度、相关权重矩阵、参数量和计算量改变',
            ],
            [
              'V：5 → 10',
              '词表有十个 Token 候选',
              'Embedding row 数和每个位置的输出 Logit 数都变为十个',
            ],
          ],
...
Output logits              [B,T,V] = [3,2,5]  每格是对 5 个 Token 的分数`,
      ...
      check(
        ...
        [
          paragraph(
            '它是第 1 条独立样本中“猫”的 Embedding row：[-0.70,0.40,0.30,0.60]。B=3 条样本、每条 T=2 个预测位置，所以共有 B×T=6 道题；Batch 只是并行装载独立样本，并没有把它们拼成同一条句子。整个 x 的标量数为 3×2×4=24。',
          ),
        ],
      ),
    ),
    section(
      ...
      '现在已经分别算过表示、分数、概率和损失。本节不再重复推导，而是检查你能否把它们接起来，并认出每个数组属于哪一步。',
      [
        '把原先分开的计算放回同一批输入。',
        '分清参数、当前中间结果和真实目标；保持六道题的排列对应。',
      ],
      [
        paragraph(
        ...
        table(
          ...
          [
            ...
            [
              'raw IDs',
              '[[0,1,2],[4,1,0],[0,3,2]]',
              '固定词表编码，每行保留三个连续 token。',
            ],
            [
              'inputs / targets',
              '[[0,1],[4,1],[0,3]] / [[1,2],[1,0],[3,2]]',
              '同一行错开一个位置，得到六道有答案的题。',
            ],
            [
              'embedding / h',
              '[3,2,4] / [3,2,4]',
              '前者查表；后者由所选模型处理可见上下文后得到。',
            ],
            ['logits', '[3,2,5]', '每道题用同一个输出层，对五个候选分别打分。'],
            ['mean loss', '一个数', '各行按对应 target 取负对数，再平均六项。'],
          ],
        ),
        paragraph(
      ...
      [
        '这一节的固定 h 是评分演示的起点，不是声称已经跑完 Attention。',
        '用概率手算便于理解，但训练函数接收 logits，不额外做一次 Softmax。',
        '展平 logits 和 targets 必须使用相同顺序。',
      ],
      check('完成串联题后核对答案。', [
        paragraph(
          '第 4 行对应 [b=2,t=0]：“我”预测“学习”。shape 只说明有四个特征，不规定怎样算出它们；上下文运算可以保持 shape 而改变数值。输出层及其上游可训练参数通过计算图收到梯度；Token ID、Target ID 是数据索引，不是需要求导的参数。回顾表示、评分和交叉熵各节。',
        ),
      ]),
    ),
    section(
      ...
      [
          ...
          table(
            ...
            [
              [
                'P(…)',
                '“某事件的概率”这一种写法',
                '模型对“下一个 Token 是什么”的条件概率规则。',
              ],
              [
                'p',
                '一个完整的概率分布向量',
                '五个候选按 Vocabulary 顺序的概率：[0.0802,0.5923,0.2179,0.0295,0.0802]。',
              ],
              ['pᵢ', 'p 的第 i 个数', '候选 i 单独成为下一个 Token 的概率。'],
              [
                'z',
                'Logits 向量',
                'Softmax 之前的五个原始分数；它们不是概率。',
              ],
              [
                'h',
                '当前位置的 Context Representation',
                '模型从允许看到的文字整理出的内部证据；它不是 Token ID，也不是概率。',
              ],
            ],
          ),
          formula(
        ...
        table(
          ...
          [
            [
              'x',
              '一串 Token（实现时通常是一串 Token IDs）',
              '“我”“喜欢”“AI”依次是 x₀、x₁、x₂。',
            ],
            [
              't',
              '当前正在给哪一个位置出题',
              '若 t=1，模型已经看到了位置 0 和 1。',
            ],
            ...
            [
              'P',
              '模型输出的概率规则',
              'P(AI｜我喜欢) 是模型给“AI”作为下一 Token 的概率。',
            ],
          ],
        ...
        callout('同一个 Batch 的六道概率题', [
          paragraph(
            '本例 B×T=3×2=6。下表使用人为指定的教学分数，不是训练日志；对同一确定性模型、同一位置且相同可见上下文的两题，必须使用同一组分数。因此 [0,0] 与 [2,0] 都看到“我”，共享 z=[0,2,1,−1,0]，只因 Target 不同而从分布中取出不同项。其余不同上下文的行用于练习计算，并不宣称这些数由一组已经训练的权重产生。',
          ),
          table(
            [
              '位置 [b,t]',
            ...
            [
              ...
              [
                ...
                '学习 / ID 3',
                'P(学习｜我)',
                'z=[0,2,1,−1,0] → p₃=0.02949 → −ln(p₃)≈3.5237',
              ],
              [
                '[2,1]',
                '我 学习',
            ...
            '每行先对五个 Logits 做 Softmax，得到五个概率；Cross Entropy 只按 Target ID 取 pᵢ，再计算 −ln(pᵢ)。例如 [2,0] 的正确 ID 是 3，取同一“我”分布里的 p₃≈0.02949。换答案不改预测分布；它改变本题对模型的评价和梯度。',
          ),
        ]),
        callout('h 通常不是“当前 Token 的原始 Embedding”', [
          ...
          paragraph(
            '评分手算给定一个与“我”的 embedding 数值相同的 h，作为输出层的教学输入；这不是完整 Transformer 的通用计算规则。这个区别很重要：两个位置都出现“喜欢”时，原始 Embedding 一样，但“我 喜欢”和“猫 喜欢”给出的 h 可以不同，后面的 Logits 与概率也就可以不同。',
          ),
        ]),
          ...
          table(
            ...
            [
              [
                ...
                'hₜ=f(x≤ₜ)，由完整可见前缀产生 Context Representation',
                'P(next｜完整可见前缀)',
              ],
              [
                '本课 Bigram',
                '直接用当前 Token ID 查一行 next-token Logits，不显式构造丰富的 h',
                'P(next｜当前 Token)',
              ],
            ],
          ),
        ...
        callout('先定义 prompt、next_id 与 append', [
          table(
            ...
            [
              [
                'prompt / history',
                '已经给模型看的 Token IDs；也就是目前的文字历史',
                '“我”的 history 是 [0]。',
              ],
              [
                'next_id',
                '本轮从概率分布中选出的一个整数 Token ID',
                '若选“喜欢”，next_id 是 [1]。',
              ],
              [
                'append（追加）',
                '把 next_id 放在 history 的末尾，形成下一轮的新输入',
                '[0] 追加 [1] 后变成 [0,1]，也就是“我 喜欢”。',
              ],
            ],
          ),
          code(
        ...
        callout('argmax 和 sampling：都基于 p，但选择规则不同', [
          table(
            ...
            [
              [
                'argmax',
                '总是选 Probability 最大的 Token；不随机',
                '最大值是 0.5923，所以选“喜欢”。',
              ],
              [
                'sampling',
                '把 p 当作抽签权重随机抽一个 Token；高概率更常抽到，但不保证',
                '“喜欢”在大量重复抽样中约占 59.23%；“AI”约占 21.79%，一次也可能被抽到。',
              ],
            ],
          ),
          paragraph(
...
T = N - 1 = 2`,
        ...
        callout('right shift 与 causal mask：两道不同的安全门', [
          table(
            ...
            [
              [
                'right shift',
                '哪一个 Token 是当前位置的正确 Target',
                '会把“复读当前 Input”错当成训练目标。',
              ],
              [
                'causal mask',
                'Transformer 在算位置 t 的 hₜ 时，可以读取输入序列中的哪些位置',
                '它可能偷看右侧 Input，而右侧 Input 恰好含有该位置的 Target。',
              ],
            ],
          ),
          paragraph(
    ...
    section(
      ...
      [
        callout('h 从哪里来：先用一个能算懂的上下文处理器', [
          paragraph(
            '上下文表示不是另一本词典，而是本次计算的结果。为了先看见“结合上下文”这个动作，暂时把允许看到的 embedding 逐项求平均。只看“我”时，平均只有一行，h 就等于 E[我]。看到“我 喜欢”时，每个坐标取两行的平均；换成“猫 喜欢”，末尾 token 不变，但另一行输入变了，结果也会变。',
          ),
          table(
            ['可见前缀', '逐项平均', '得到的 h'],
            [
              ['我', '只有 E[我] 一行', '[0.20,−0.10,0.70,0.30]'],
              ['我 喜欢', '(E[我]+E[喜欢])/2', '[0.40,0.10,0.25,0.20]'],
              ['猫 喜欢', '(E[猫]+E[喜欢])/2', '[−0.05,0.35,0.05,0.35]'],
            ],
          ),
          paragraph(
            '例如“我 喜欢”的第 2 个坐标是 (−0.10+0.30)/2=0.10；同位置的“猫 喜欢”则是 (0.40+0.30)/2=0.35。一次计算只用输入，不看真实下一词。这些数字不是对“喜欢”的另一种永久编号，而是针对当前前缀整理出的临时信息。',
          ),
          paragraph(
            '平均只是打开 h 来源的临时教学模型：它让所有可见位置同权，不能表达“本次更需要哪一处信息”。Week 7 的 Attention 会用可学习权重改进这一点，Week 8 再加其他处理。它既不是只查当前分数的 Bigram，也不是完整 GPT。下面先选最简单的“我”这一行，给同一个 h 的五个候选打分。',
          ),
        ]),
        ...
        callout('用很小的改动核对正负号', [
          table(
            ...
            [
              ...
              [
                '把正确 z_喜欢 从 2 增至 2.01',
                '0.594711',
                '0.519679',
                'Loss 下降，符合 g_喜欢<0。',
              ],
              [
                '把错误 z_AI 从 1 增至 1.01',
                '0.591005',
                '0.525932',
                'Loss 上升，符合 g_AI>0。',
              ],
          ...
          table(
            ...
            [
              [
                'g=dL/dz',
                '[V]=[5]',
                '[0.0802,−0.4077,0.2179,0.0295,0.0802]：五个候选 Logit 的方向。',
              ],
              [
                'dL/db_喜欢',
                'scalar',
                'g_喜欢=−0.4077；执行减法更新时，喜欢的 Bias 会上升。',
              ],
              [
                'dL/dw_喜欢',
                '[C]=[4]',
                'g_喜欢×h=−0.4077×[0.20,−0.10,0.70,0.30]≈[−0.0815,0.0408,−0.2854,−0.1223]。',
              ],
              [
                'dL/dh',
                '[C]=[4]',
                'W_outᵀg：把五个候选的反馈汇总，交给产生 h 的 Context Model。',
              ],
            ],
          ),
          code(
      ...
      [
        ...
        callout('Bigram 在整套课程中负责什么？', [
          table(
            ...
            [
              [
                '把当前 Token ID 映射为 V 个 next-token Logits',
                '直接复用已经学过的 ID、Logits、Softmax、Cross Entropy 与 Gradient。',
              ],
              [
                '根据训练文本中的相邻 Token 对，调整每一行 Logits',
                '能用很少的参数和很小的数据跑完一次真实训练。',
              ],
              [
                '从最后一个 Token 的分布选择并 append 新 Token',
                '能实际生成，而不是只有公式或分类表。',
              ],
              [
                '故意忽略更早上下文',
                '它会在需要区分“我 喜欢”和“猫 喜欢”时失败；失败本身引出 Attention 的必要性。',
              ],
            ],
          ),
          paragraph(
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, token_ids, targets=None):
        ...
        callout('Bigram 的能力边界正好说明 Attention 为什么存在', [
          table(
            ...
            [
              ...
              [
                '比较“我 喜欢”与“猫 喜欢”后面是什么',
                '两次都只有当前 ID=喜欢',
                '必定使用同一 row 1 与同一分布，无法按更早前缀区分。',
              ],
              [
                '预测“喜欢”之后是什么',
                '当前 ID=喜欢',
                '只能折中适应训练中 AI 与我两种答案，不能知道此刻来自哪条短句。',
              ],
            ],
          ),
          paragraph(
      ...
      check('Bigram 是做什么的？为什么它无法区分“我 喜欢”与“猫 喜欢”？', [
        paragraph(
        ...
        callout('先摆清桌上的三样东西', [
          table(
            ...
            [
              [
                'inputs：题目',
                '[[我,喜欢],[猫,喜欢],[我,学习]]，shape [3,2]',
                '本节固定这六道题，循环中保持不变。',
              ],
              [
                'targets：标准答案',
                '[[喜欢,AI],[喜欢,我],[学习,AI]]，shape [3,2]',
                '来自原始文本，模型不能修改答案。',
              ],
              [
                'model.token_table.weight：评分规则',
                '5 行×5 列的 Logits 参数表',
                'Optimizer 每一步修改它；这是学习真正发生的位置。',
              ],
            ],
          ),
          paragraph(
